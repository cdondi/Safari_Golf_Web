...
Partial Class WebUserControl
    ...
    Protected Sub grdViewScorecard2_DataBound(sender As Object, e As EventArgs)
        'Starting from last Row Previous Row (if 12 rows in our grid)
        ' merging last row and last previous row if same value (take 12, 11 and merge if same year)
        ' Then last previous row with second last previous row (take 11, 10 and merge if same year)
        ' etc., till first row and second row merging
        For rowIndex As Integer = grdViewScorecard2.Rows.Count - 2 To 0 Step -1
            Dim row As GridViewRow = grdViewScorecard2.Rows(rowIndex)
            Dim previousRow As GridViewRow = grdViewScorecard2.Rows(rowIndex + 1)
            If row.Cells(0).Text = previousRow.Cells(0).Text Then
                If previousRow.Cells(0).RowSpan < 2 Then
                    row.Cells(0).RowSpan = 2
                Else
    ...
    Protected Sub grdViewScorecard2_RowDataBound(sender As Object, e As GridViewRowEventArgs)
        If e.Row.RowType = DataControlRowType.Header Then
            'e.Row.Cells(0).Visible = False
            '' Invisibiling Year Header Cell
            'e.Row.Cells(1).Visible = False
        ...
        End If
    End Sub
    Protected Sub grdViewScorecard2_RowCreated(sender As Object, e As GridViewRowEventArgs)
    ...
    Private Sub grdViewScorecard1BindData()
        ...
        strQuery = "SELECT  [tb_Scorecard].[ScorecardClosed] as scorecardsclosed, [tb_Scorecard].[ScorecardId] as ScorecardId, [tb_Scorecard].[GhinID] as [GhinID],[tb_Scorecard].[memberid] as memberid, [memberinfo].[firstname] + ' ' + [memberinfo].[lastname] as playername,[Score1] ,[Score2] ,[Score3] ,[Score4] ,[Score5]   ,[Score6]  ,[Score7] ,[Score8]  ,[Score9]  ,[ScoreTotalout] FROM  [tb_Scorecard], [memberinfo] where [tb_Scorecard].[memberid] =[memberinfo].[memberid] and [PairingID] =" & ScorecardPairID & " and [LocationID]  = " & LocationID() & " and [Date] = '" & DateID() & "' order by [tb_Scorecard].[GhinID]"
    ...
    Private Sub grdViewScorecard2BindData()
        ...
        Dim strQuery As String = "SELECT  [tb_Scorecard].[ScorecardClosed] as scorecardsclosed, [tb_Scorecard].[ScorecardId] as ScorecardId, [tb_Scorecard].[GhinID] as [GhinID],[tb_Scorecard].[memberid] as memberid, [memberinfo].[firstname]  + ' ' +  [memberinfo].[lastname] as playername ,[Score10],[Score11] ,[Score12]  ,[Score13],[Score14]   ,[Score15]    ,[Score16] ,[Score17]  ,[Score18] ,[ScoreTotalin] ,[GrandTotal],[memberinfo].[TournamentHandicap]  ,[memberinfo].[GhinHandicap]  ,[NetScore] FROM  [tb_Scorecard], [memberinfo] where [tb_Scorecard].[memberid] =[memberinfo].[memberid] and [PairingID] =" & ScorecardPairID & " and [LocationID]  = " & LocationID() & " and [Date] = '" & DateID() & "'  order by [tb_Scorecard].[GhinID]"

        Dim cmd As New SqlCommand(strQuery)
        grdViewScorecard2.DataSource = GetData(cmd)
        grdViewScorecard2.DataBind()
    ...
    Protected Sub grdViewScorecard1UpdateScore(ByVal sender As Object, ByVal e As GridViewUpdateEventArgs)
        ...
        Dim lblscorecardid As String = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("lblScorecardID"), Label).Text

        If GhinID <> "" And lblscorecardid <> "" Then
            ...
            updateScoreCard1(GhinID, lblscorecardid, lbl1.SelectedValue, lbl2.SelectedValue, lbl3.SelectedValue, lbl4.SelectedValue, lbl5.SelectedValue, lbl6.SelectedValue, lbl7.SelectedValue, lbl8.SelectedValue, lbl9.SelectedValue)
    ...
    Protected Sub grdViewScorecard2OnPaging(ByVal sender As Object, ByVal e As GridViewPageEventArgs)
        grdViewScorecard2BindData()
        grdViewScorecard2.PageIndex = e.NewPageIndex
        grdViewScorecard2.DataBind()
    End Sub

    Protected Sub grdViewScorecard2EditScore(ByVal sender As Object, ByVal e As GridViewEditEventArgs)
        grdViewScorecard2.EditIndex = e.NewEditIndex
        grdViewScorecard2BindData()
    End Sub
    Protected Sub grdViewScorecard2CancelEdit(ByVal sender As Object, ByVal e As GridViewCancelEditEventArgs)
        grdViewScorecard2.EditIndex = -1
        grdViewScorecard2BindData()
    End Sub
    Protected Sub grdViewScorecard2UpdateScore(ByVal sender As Object, ByVal e As GridViewUpdateEventArgs)

        Dim GhinID As String = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("lblGhinID"), Label).Text
        Dim lblscorecardid As String = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("lblScorecardID"), Label).Text

        If GhinID <> "" And lblscorecardid <> "" Then

            Dim lbl10 As DropDownList = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("ddl10"), DropDownList)
            Dim lbl11 As DropDownList = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("ddl11"), DropDownList)
            Dim lbl12 As DropDownList = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("ddl12"), DropDownList)
            Dim lbl13 As DropDownList = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("ddl13"), DropDownList)
            Dim lbl14 As DropDownList = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("ddl14"), DropDownList)
            Dim lbl15 As DropDownList = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("ddl15"), DropDownList)
            Dim lbl16 As DropDownList = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("ddl16"), DropDownList)
            Dim lbl17 As DropDownList = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("ddl17"), DropDownList)
            Dim lbl18 As DropDownList = DirectCast(grdViewScorecard2.Rows(e.RowIndex).FindControl("ddl18"), DropDownList)

            updateScoreCard2(GhinID, lblscorecardid, lbl10.SelectedValue, lbl11.SelectedValue, lbl12.SelectedValue, lbl13.SelectedValue, lbl14.SelectedValue, lbl15.SelectedValue, lbl16.SelectedValue, lbl17.SelectedValue, lbl18.SelectedValue)
    ...
    Private Sub updateScoreCard1(ByVal GhinID As String, ByVal scorecardid As String, ByVal Score1 As String, ByVal Score2 As String, ByVal Score3 As String, ByVal Score4 As String, ByVal Score5 As String, ByVal Score6 As String, ByVal Score7 As String, ByVal Score8 As String, ByVal Score9 As String)
        Dim i As Integer = 1
    ...
    Private Sub updateScoreCard2(ByVal GhinID As String, ByVal scorecardid As String, ByVal Score10 As String, ByVal Score11 As String, ByVal Score12 As String, ByVal Score13 As String, ByVal Score14 As String, ByVal Score15 As String, ByVal Score16 As String, ByVal Score17 As String, ByVal Score18 As String)
