...
Partial Class PlayerPerformanceControl
    ...
    Private Sub BindClosedGrid()
        Dim i As Integer = 1
        Dim ssqlr As String = "Select distincT([tb_Scorecard].[Date]) as dtx FROM [tb_Scorecard] where [LocationID] = 1 and [tb_Scorecard].[ScorecardClosed] = 'True' order by [Date] desc"
    ...
    Private Sub GetClosedGametoGrid(DateString As String, gridnum As Integer)
        ...
        Dim query As String = "SELECT  [tb_Scorecard].[GhinID] ,[MemberInfo].[firstname] ,[MemberInfo].[lastname] ,[tb_Scorecard].[Date], [tb_Scorecard].[ScoreTotalout],[tb_Scorecard].[ScoreTotalin],[tb_Scorecard].[GrandTotal],[tb_Scorecard].[TournamentHandicap],[tb_Scorecard].[GhinHandicap]  FROM [tb_Scorecard],[MemberInfo] where [MemberInfo].[GhinID]= [tb_Scorecard].[GhinID] and [tb_Scorecard].[ScorecardClosed] = 'True' and [tb_Scorecard].[locationid] = 1  AND [tb_Scorecard].[Date] ='" & DateString & "'"
