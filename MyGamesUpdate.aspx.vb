...
Partial Public Class Events_Calendar
    ...
    '        Dim id As Object = ViewState("memID")
    '        If id IsNot Nothing Then
    '            m_memID = CInt(id)
    '        Else
    '            If id IsNot Nothing Then
    '                m_memID = Convert.ToString(id)
    '            Else
    '                m_memID = getmemID()
    '            End If
    '            ViewState("memID") = m_memID
    '        End If
    '        Return m_memID
    '    End Get
    '    Set(value As String)
    '        ViewState("memID") = value
    '    End Set
    'End Property
    ...
    End Sub
    Private Function getmemID() As String

        Dim UserId As String = "xxxxxxxxxxx"
    ...
    Private Sub getmemberlocations()
        ...
        Try
            ...
            If Not currentUser Is Nothing Then
                ...
                SqlDataSource1.SelectCommand = "SELECT  [tb_Scorecard].[LocationID],[Locations].[title] as locationname  , (select CONVERT(VARCHAR(8), [Date], 1) AS [MM/DD/YYYY]) as Dateid FROM [tb_Scorecard] ,[Locations] where  [tb_Scorecard].[LocationID] =  [Locations].[ID] and  [tb_Scorecard].[memberid] ='" & UserId & "'  group by  locationid , [Locations].[title] , [Date]  order by [Date] desc, locationid"
    ...
    Private Sub grdViewScorecard1BindData()
        ...
        Else
            ...
            If UserId <> "" Then
                ...
                strQuery = "SELECT  [tb_Scorecard].[ScorecardClosed] as scorecardsclosed, [tb_Scorecard].[ScorecardId] as ScorecardId, [tb_Scorecard].[GhinID] as [GhinID],[tb_Scorecard].[memberid] as memberid, [memberinfo].[firstname] + ' ' + [memberinfo].[lastname] as playername,[Score1] ,[Score2] ,[Score3] ,[Score4] ,[Score5]   ,[Score6]  ,[Score7] ,[Score8]  ,[Score9]  ,[Score10],[Score11] ,[Score12]  ,[Score13],[Score14]   ,[Score15]    ,[Score16] ,[Score17]  ,[Score18] FROM  [tb_Scorecard], [memberinfo] where [tb_Scorecard].[memberid] =[memberinfo].[memberid]  and [memberinfo].[memberid] ='" & UserId & "' and [Date] ='" & DateID & "' and [LocationID]  = " & LocationID & " order by [memberinfo].[lastname],[memberinfo].[firstname] "
